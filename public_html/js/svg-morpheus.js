...
function transCalc(transFrom, transTo, progress) {
  ...
  for(var i in transFrom) {
    switch(i) {
      ...
      res[i]=[0,0];
      for(var j=0;j<2;j++) {
        res[i][j]=parseFloat(transFrom[i][j])+(parseFloat(transTo[i][j])-parseFloat(transFrom[i][j]))*progress;
      }
      break;
    }
  }
...
function trans2string(trans) {
  ...
  if (!!trans.translate) {
    res+='translate('+trans.translate.join(',')+') ';
  }
  return res;
}
...
function SVGMorpheus(element, options, callback) {
  ...
  this._morphG;
  this._startTime;
  this._defDuration=options.duration || 1200;
  ...
  if(!!this._svgDoc) {
    ...
    for(i=this._svgDoc.childNodes.length-1;i>=0;i--) {
      ...
      if(nodeIcon.nodeName.toUpperCase()==='G') {
        ...
        var origTransform=nodeIcon.getAttribute('transform');
            ...
            if(item.path!='') {
              ...
              for (var k = 0, len3=nodeItem.attributes.length; k < len3; k++) {
                ...
                if (attrib.specified) {
                  ...
                  if (name=='transform') {
                    item.origTransform=attrib.value;
                  }
                }
              }
...
SVGMorpheus.prototype._setupAnimation=function(toIconId) {
  if(!!toIconId && !!this._icons[toIconId]) {
    ...
    // $(this._morphG).animate({'svgTransform':this._icons[toIconId].origTransform},700);
    // this._morphG.setAttribute('transform',this._icons[toIconId].origTransform);
      ...
      if(!this._fromIconItems[i]) {
        if(!!this._toIconItems[i]) {
          ...
          this._fromIconItems.push({
            ...
            attrs: {},
            style: {},
            origTransform: this._toIconItems[i].origTransform,
            trans: {
      ...
      if(!this._toIconItems[i]) {
        if(!!this._fromIconItems[i]) {
          ...
          this._toIconItems.push({
            ...
            attrs: {},
            style: {},
            origTransform: this._fromIconItems[i].origTransform,
            trans: {
              'rotate': [0,toBB.cx,toBB.cy]
            }
          });
...
SVGMorpheus.prototype._updateAnimationProgress=function(progress) {
  ...
  for(i=0, len=this._morphNodes.length;i<len;i++) {
    ...
    // if (this._curIconItems[i].origTransform) morphNode.node.setAttribute("transform",this._curIconItems[i].origTransform);
  }
  curTran=-1;
  if (this._icons[this._curIconId]) {
    var curTran=(this._icons[this._curIconId].origTransform+'').split(' ');
    var curTrans=[];
    for (var i in curTran) {
      var x=curTran[i].split('(');
        curTrans[x[0]]=x[1].substr(0,x[1].length-1).replace(' ','').split(',');
      }
    } 

    var toTran=(this._icons[this._toIconId].origTransform+'').split(' ');
    var toTrans=[];
    for (var i in curTran) {
      var x=toTran[i].split('(');
        toTrans[x[0]]=x[1].substr(0,x[1].length-1).replace(' ','').split(',');
      }
      if (curTran==-1) curTrans=toTrans;
      var trans=trans2string(transCalc(curTrans, toTrans, progress));
    this._morphG.setAttribute('transform',trans);
