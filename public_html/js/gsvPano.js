...
GSVPANO.PanoLoader = function (parameters) {
  ...
  this.composePanorama = function () {
    ...
    for (var y = 0; y < h; y++) {
      for (var x = 0; x < w; x++) {
        var url =
          "https://cbks2.google.com/cbk?cb_client=maps_sv.tactile&authuser=0&hl=en&panoid=" +
          _panoId +
          "&output=tile&zoom=" +
          _zoom +
          "&x=" +
  ...
  this.load = function (location) {
    ...
    var url =
      "https://maps.googleapis.com/cbk?output=json&hl=x-local&ll=" +
      location.lat() +
      "," +
      location.lng() +
      "&cb_client=maps_sv&v=3";
    // var url = 'https://maps.google.com/cbk?output=json&hl=x-local&ll=' + location.lat() + ',' + location.lng() + '&cb_client=maps_sv&v=3';
    // url = 'https://cbks0.google.com/cbk?cb_client=maps_sv.tactile&authuser=0&hl=en&output=polygon&it=1%3A1&rank=closest&ll=' + location.lat() + ',' + location.lng() + '&radius=5000';

    const sv = new google.maps.StreetViewService();
    sv.getPanorama({ location: location, radius: 50 }).then((r) =>
      self.processSVData(r)
    );
  };

  this.processSVData = function ({ data }) {
    var self = this;
    ...
    if (typeof data.result != "undefined") {
      ...
      jQuery(document.body).removeClass("mapping in-street");
    }
    /// }
    /// };
